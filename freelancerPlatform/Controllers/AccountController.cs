...
namespace freelancerPlatform.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                string img = Path.GetFileNameWithoutExtension(model.ImageFile.FileName);
                string ex = Path.GetExtension(model.ImageFile.FileName);
                img = img + ex;
                model.Image = "~/Content/images/" + img;
                img = Path.Combine(Server.MapPath("~/Content/images/"), img);
                model.ImageFile.SaveAs(img);
                ...
                if (result.Succeeded)
                {
                    ...
                    else if(role=="freelancer")
                    {
                        ...
                        //var model1 = db.Users.SingleOrDefault(u => u.Email == User.Identity.Name.ToString());
